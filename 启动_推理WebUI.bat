...
set "STAMP=%STAMP: =0%"
set "STAMP=%STAMP:,=-%"
set "LOG_FILE=%cd%\logs\startup\infer-webui-%STAMP%.log"

echo [INFO] 推理 WebUI 启动预检，日志: %LOG_FILE%
"%PYTHON_EXECUTABLE%" -u tools\startup_check.py --mode infer-webui >> "%LOG_FILE%" 2>&1
if errorlevel 1 (
    type "%LOG_FILE%"
...
echo [INFO] 推理 WebUI: http://127.0.0.1:9872/
echo [INFO] 推理 WebUI: http://127.0.0.1:9872/ >> "%LOG_FILE%"
"%PYTHON_EXECUTABLE%" -s -u tools\run_with_bootstrap.py -- GPT_SoVITS\inference_webui.py zh_CN >> "%LOG_FILE%" 2>&1
pause
