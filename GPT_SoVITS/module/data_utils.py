...
class TextAudioSpeakerLoader(torch.utils.data.Dataset):
    ...
    def __init__(self, hparams, version=None,val=False):
        ...
        if leng == 0:
            ...
            cnt5 = len(names5)
            cnt7 = len(names6)
            cnt2 = len(self.phoneme_data)
            raise RuntimeError(
                f"Empty dataset at {exp_dir}: text={cnt2}, hubert={cnt4}, wav32k={cnt5}"
                + (f", sv={cnt7}" if self.is_v2Pro else "")
                + ". Generate datasets and ensure filenames match (with .wav)."
            )
        if leng < min_num:
            self.audiopaths_sid_text = []
            for _ in range(max(2, int(min_num / max(1, leng)))):
